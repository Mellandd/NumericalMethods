...
public class AdaptativeStepRKF45Method extends AdaptativeStepMethod{
	...
	protected double doStep(double time, double[] state) {
		...
		while(!accepted) {
			this.addToEvaluationCounter(6);
			double[] k1 = mProblem.getDerivative(time, state);
	        IntStream.range(0, state.length).forEach(i-> k1[i] = mStep*k1[i]);
	        ...
	        IntStream.range(0, state.length).forEach(i-> k2[i] = mStep*k2[i]);
	        ...
	        IntStream.range(0, state.length).forEach(i-> k3[i] = mStep*k3[i]);
	        ...
	        IntStream.range(0, state.length).forEach(i-> k4[i] = mStep*k4[i]);
	        ...
	        IntStream.range(0, state.length).forEach(i-> k5[i] = mStep*k5[i]);
	        ...
	        IntStream.range(0, state.length).forEach(i-> k6[i] = mStep*k6[i]);
	        IntStream.range(0, state.length).forEach(i->stateRK4[i] = state[i]+(25./216.)*k1[i]+(1408./2565.)*k3[i]+(2197./4104.)*k4[i]-(1./5.)*k5[i]);
